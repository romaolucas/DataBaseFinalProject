...
public class ProviderLoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			List<ProviderBean> providerList = providerDAO.getProviders(email,password);
			log.info(Integer.toString(providerList.size()));
			if(providerList.size()>0){
				   loginSuccessful=true;
				   boolean hasApplication = providerDAO.hasApplication(providerList.get(0));
				   ...
				   Cookie userEmail = new Cookie("email", providerList.get(0).getEmail());
		...
		if (loginSuccessful) {
			RequestDispatcher dispatcher = request.getRequestDispatcher("/welcome-provider.jsp");
