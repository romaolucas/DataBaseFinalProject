...
public class ProviderDAO {
	...
	private StringBuffer getPasswordMD5(String password) throws NoSuchAlgorithmException {
	...
	public List<ProviderBean> getProviders(String email, String password) {
		...
		try {
			...
			preparedStatement = connection.prepareStatement(PGUtils.fetchProviderCredentials);
	...
	public List<ProviderBean> getProviders(String email) {
		...
		try {
			...
			// Fetch supplier key from the supplier table using the supplier
			// name that user provides as input
			preparedStatement = connection.prepareStatement(PGUtils.fetchProviderCredentialsByEmail);
			preparedStatement.setString(1, email);
			resultSet = preparedStatement.executeQuery();
			if (resultSet.next()) {
				do {
					...
					providerBean.setEmail(resultSet.getString("email"));
					providerBean.setPassword(resultSet.getString("password"));
					String getBand = "select * from band where pid = " + Integer.toString(providerBean.getId());
					ResultSet rsGetBand;
					...
					if (rsGetBand.next()) {
						...
						String getSponsor = "select * from sponsor where pid = " + Integer.toString(providerBean.getId());	
	...
	public boolean hasApplication(ProviderBean provider) {
		Connection con;
		String query = "select pid, status, category from application where pid = " + Integer.toString(provider.getId());
		ResultSet rs;
		ResultSet rsStatus;
		Statement stmt;
	...
	public List<TimeSlotBean> getTimeSlots(int id) {
		Connection connection = null;
		PreparedStatement preparedStatement;
		ResultSet resultSet;
		...
		try {
			...
			// Fetch supplier key from the supplier table using the supplier
			// name that user provides as input
			preparedStatement = connection.prepareStatement(PGUtils.fetchTimeSlots);
			...
			if (resultSet.next()) {
				do {
					...
					timeslotBean.setTimebuildup(resultSet.getTimestamp("timebuildup"));
					timeslotBean.setTimeplay(resultSet.getTimestamp("timeplay"));
					timeslotBean.setTimefinish(resultSet.getTimestamp("timefinish"));
					timeslotBean.setTimegone(resultSet.getTimestamp("timegone"));
					String getStage = "select * from section where sectionid = " + timeslotBean.getSectionid();
					...
					if (rsGetStage.next()) {
						...
						String getAddress = "select * from area where areaid = " +areaid;
						...
						if(rsGetAddress.next()){
							String stageaddress = rsGetAddress.getString("address");
	...
	public List<InstructionBean> getInstructions(int id) {
		...
		try {
			...
			// Fetch supplier key from the supplier table using the supplier
			// name that user provides as input
			preparedStatement = connection.prepareStatement(PGUtils.fetchInstructions);
			preparedStatement.setInt(1, id);
			resultSet = preparedStatement.executeQuery();
			if (resultSet.next()) {
				do {
					...
					instructionBean.setDescription(resultSet.getString("description"));
					instructionBean.setStatus(resultSet.getString("status"));
					instructionBean.setType(resultSet.getString("type"));
					instructionBean.setCreationdate(resultSet.getTimestamp("date"));
					String getAssignment = "select * from assignment where instructionid = " + instructionBean.getId();
					ResultSet rsGetAssignment;
					Statement stmtGetAssignment;
					stmtGetAssignment = connection.createStatement();
					rsGetAssignment = stmtGetAssignment.executeQuery(getAssignment);
					if (rsGetAssignment.next()) {
						instructionBean.setEmpid(rsGetAssignment.getInt("eid"));
						instructionBean.setAssigneddate(rsGetAssignment.getTimestamp("date"));
						String getName = "select * from employee where eid = " +instructionBean.getEmpid();
						ResultSet rsGetName;
