...
public class ViewPerformanceInfoServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			List<TimeSlotBean> timeslots = providerDAO.getTimeSlots(providerList.get(0).getId());
			if(timeslots.size()>0){
				request.setAttribute("timeslots", timeslots);
				RequestDispatcher dispatcher = request.getRequestDispatcher("/view-performance-info.jsp");
